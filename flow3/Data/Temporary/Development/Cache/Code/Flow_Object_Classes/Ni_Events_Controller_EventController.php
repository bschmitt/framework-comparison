...
class EventController_Original extends ActionController
{
 ...
 * 
 */
class EventController extends EventController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		if ('Ni\Events\Controller\EventController' === get_class($this)) {
	...
	$reflectedClass = new \ReflectionClass('Ni\Events\Controller\EventController');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if ($reflectionService->isPropertyTaggedWith('Ni\Events\Controller\EventController', $propertyName, 'transient')) continue;
	 ...
	 */
	 private function Flow_Proxy_injectProperties() {
		$this->injectSettings(\TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('TYPO3\Flow\Configuration\ConfigurationManager')->getConfiguration(\TYPO3\Flow\Configuration\ConfigurationManager::CONFIGURATION_TYPE_SETTINGS, 'Ni.Events'));
		$eventRepository_reference = &$this->eventRepository;
		$this->eventRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getInstance('Ni\Events\Domain\Repository\EventRepository');
		if ($this->eventRepository === NULL) {
			$this->eventRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getLazyDependencyByHash('01addfbc899955cf399d2d22f254042a', $eventRepository_reference);
			if ($this->eventRepository === NULL) {
				$this->eventRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->createLazyDependency('01addfbc899955cf399d2d22f254042a',  $eventRepository_reference, 'Ni\Events\Domain\Repository\EventRepository', function() { return \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('Ni\Events\Domain\Repository\EventRepository'); });
			}
		}
